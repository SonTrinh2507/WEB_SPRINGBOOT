...
function ready(){
    ...
    // var btn_size = document.getElementsByClassName('psize');
    // for(var i = 0; i < btn_size.length; i++){
    //     var btn = btn_size[i];
    //     btn.addEventListener('click', choiceSize);
    // }
	 // Staff delivery
...
function next(event){
    ...
    }else if (three.className === "progress three active" && four.className !== "progress four active"){
...
for (var i = 0; i < card.length; i++){
    var percent = card[i].getElementsByClassName("discount-percent")[0];
    if(percent && percent.innerText !== ''){
        percent.innerText = '-' + percent.innerText
        var price_Discount = card[i].getElementsByClassName("dicount-price")[0];
        price_Discount.style.display = "none";
    }else{
        percent.classList.remove('discount-percent');
    }
} 
    //products-detail
